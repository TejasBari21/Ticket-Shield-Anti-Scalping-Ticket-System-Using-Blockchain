...
async function cleanSeedDatabase() {
  try {
    // Connect to MongoDB
    await mongoose.connect(MONGODB_URI);
    console.log('✅ Connected to MongoDB');

    // Force delete ALL existing users and KYC
    console.log('🗑️  Deleting all existing users and KYC...');
    const deleteUsers = await User.deleteMany({});
    const deleteKYC = await KYC.deleteMany({});
    console.log(`   Deleted ${deleteUsers.deletedCount} users`);
    console.log(`   Deleted ${deleteKYC.deletedCount} KYC records`);

    // Create sample users with correct passwords
    ...
    const kycSubmissions = await KYC.create([
      {
...
Next:
  1. Keep API server running (npm run dev)
  2. Open http://localhost:8080/login
  3. Try logging in with admin credentials above
    `);

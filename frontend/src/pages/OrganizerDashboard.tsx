...
const OrganizerDashboard = () => {
  ...
  // Refresh whenever localStorage changes (e.g. ticket purchase in same session)
  useEffect(() => {
                ...
                <div key={event.id} className="glass rounded-xl p-5 flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between hover:border-primary/30 transition-colors">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button variant="ghost" size="sm" onClick={async () => {
                      if (!confirm("Cancel this event? This will mark it as cancelled and prevent ticket sales.")) return;
                      try {
                        localDB.updateEventStatus(event.id, "cancelled");
                        toast({ title: "Event Cancelled", description: `${event.name} marked as cancelled.` });
                        loadEvents();
                      } catch (err) {
